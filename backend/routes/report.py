...
from models import Dispatch, Supply, Supplier, Product, db
...
def get_dispatch_report():
    ...
    # FIX: return flat list — no nested "data" key
    return api_response(
        status_code=200,
        message="Dispatch report retrieved successfully",
        data=[
            {
                "id": d.id,
                ...
                "date_dispatched": (
                    d.date_dispatched.strftime("%Y-%m-%d")
                    if d.date_dispatched
                    else None
                ),
            }
            for d in dispatches
...
def get_supply_report():
    ...
    # FIX: query Supply model, not Dispatch
    query = Supply.query

    if start_date:
        query = query.filter(Supply.date_supplied >= start_date)
    if end_date:
        query = query.filter(Supply.date_supplied <= end_date)

    supplies = query.order_by(Supply.date_supplied.desc()).all()

    return api_response(
        status_code=200,
        message="Supply report retrieved successfully",
        data=[
            {
                "id": s.id,
                ...
                "date_supplied": (
                    s.date_supplied.strftime("%Y-%m-%d")
                    if s.date_supplied
                    else None
                ),
                "remarks": s.remarks,
...
def max_sold_product():
    ...
    return api_response(
        status_code=200,
        message="Max sold product retrieved successfully",
        data={
...
def frequent_party():
    ...
    party = Supplier.query.get(result.party_id)

    return api_response(
        status_code=200,
        message="Frequent party retrieved successfully",
        data={
            "party_id": party.id,
